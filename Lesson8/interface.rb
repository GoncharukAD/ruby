...
class Interface
  ...
  def set_route
    puts 'Введите номер поезда  из списка'
    @trains.each { |_train| puts t.number }
    number = gets.chomp
  ...
  def trains_on_station
    ...
    @stations.each { |_station| puts name }
    station_name = gets.chomp
  ...
  def load_wagon
    @wagons.each { |w| puts w.number if w.instance_of?(CargoWagon) }
    puts 'Введите номер вагона.Допустимый формат: 2 буквы и 3 цифры,без дефисов и пробелов'
    wagon_number = gets.chomp
    if @wagons.include?(wagon_number)
      puts 'Введите количество груза'
      amount = gets.chomp.to_i
      @wagons.each { |_n, w| w.load_wagon(amount) && w.occupied_volume if w.number == wagon_number }
    else
  ...
  def take_a_seat
    @wagons.each { |w| puts w.number if w.instance_of?(PassengerWagon) }
    puts 'Введите номер вагона.Допустимый формат: 2 буквы и 3 цифры,без дефисов и пробелов'
    wagon_number = gets.chomp
    if @wagons.include?(wagon_number)
